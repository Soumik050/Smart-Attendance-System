...
class Face_Recognition:
    ...
    def __init__(self, root):
        ...
        # title section
        title_lb1 = Label(bg_img, text="Welcome to Face Recognition Pannel", font=(
    ...
    def face_recog(self):
        def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in featuers:
                ...
                sid, predict = clf.predict(gray_image[y:y+h, x:x+w])
                ...
                cursor.execute(
                    "select Name from student where Student_ID="+str(sid))
                n = cursor.fetchone()
                print("n : ", n)
                n = "+".join(n)

                cursor.execute(
                    "select Roll_No from student where Student_ID="+str(sid))
                r = cursor.fetchone()
                print("r : ", r)
                r = "+".join(r)

                cursor.execute(
                    "select Department from student where Student_ID="+str(sid))
                i = cursor.fetchone()
                print("i : ", i)
                i = "+".join(i)
        ...
        while True:
            ...
            cv2.imshow("Face Detector (press q to close)", img)
